...
function downloadLogsFromServer(serverConfig, logFileName) {
  ...
  try {
    ...
    const downloadCommand = `sshpass -p '${serverConfig.password}' scp -P ${serverConfig.port} -o StrictHostKeyChecking=no -o LogLevel=QUIET ${serverConfig.user}@${serverConfig.host}:${logFileName} ${tempLogPath}`;
...
async function uploadLogs(chatId, topicId) {
  ...
  try {
    ...
    try {
      ...
      const cleanupCommand = `sshpass -p '${serverConfig.password}' ssh -p ${serverConfig.port} -o StrictHostKeyChecking=no -o LogLevel=QUIET -T ${serverConfig.user}@${serverConfig.host} "rm -f ${LOG_FILE_PATH}"`;
